...
void PowerSaveTimer::PowerSaveCheck() {
    ...
    // Simplified power save check for alarm clock - only sleep when idle
    if (!in_sleep_mode_ && device_state != kDeviceStateIdle) {
        ESP_LOGD(TAG, "Not entering sleep mode - device not idle");
        return;
    }

    // Continue with existing power save logic...
    ticks_++;
